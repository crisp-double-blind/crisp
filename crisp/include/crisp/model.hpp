...
void save_model(model_t const& m, path_t const& file_name);
std::unique_ptr<model_t> load_model(path_t const& file_name);
std::unique_ptr<model_t> load_model(
  path_t const& json_path, std::string const& scenario_id,
  path_t const& project_root = path_t());
}  // namespace crisp
